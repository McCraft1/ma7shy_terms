...
bot.on('ready', () => {
    ...
    // ضبط الحالة المخصصة تظهر على الموبايل والكمبيوتر
    bot.user.setPresence({
        activities: [{ 
            name: 'customstatus', 
            type: 4, // النوع 4 مخصص للحالة النصية المخصصة (Custom Status)
            state: 'Eren Is The best' // الكلمة اللي طلبتها بالظبط
        }],
        status: 'online',
    });
});
...
bot.on('messageCreate', async (message) => {
    ...
    if (command === 'panel') {
        ...
        const oauthUrl = `https://discord.com/oauth2/authorize?client_id=${CLIENT_ID}&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&response_type=code&scope=identify%20messages.read%20rpc%20rpc.act[...]
